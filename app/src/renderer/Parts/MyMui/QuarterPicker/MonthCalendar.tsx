...
>(inProps: MonthCalendarProps<TDate>, ref: React.Ref<HTMLDivElement>) {
  ...
  console.log('in m', inProps.value)
  ...
  return (
    ...
    >
      {getMonthsInYear(utils, value ?? referenceDate).map((month) => {
        ...
        const selectedQuater =
          Math.floor(monthNumber / 3) * 3 +
          (selectedMonth ? selectedMonth % 3 : 0)
        const isSelected = monthNumber === selectedMonth
        const isDisabled = disabled || isMonthDisabled(month)
        if (monthNumber != selectedQuater) return
          ...
          >
            ...
            <>
              <div style={{ display: 'flex' }}>
                {[0, 1, 2].map((i) => (
                  <span
                    style={{ width: 72, flexBasis: `${25 * ((i % 2) + 1)}%` }}
                    key={monthNumber + i}
                  >
                    {Math.floor(monthNumber / 3) * 3 + (i + 1)}
                  </span>
                ))}
              </div>
            </>
